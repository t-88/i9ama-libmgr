...
function BookAvaible({ book }: { book: Book }) {
    ...
    if (book.available || !booked_info ) return <>/</>
...
function Books() {
    ...
    function onSearch(author: string,year: string,tags : string[]) {
        const input = searchRef.current.getInput() as string;
        setFilter(input.toLowerCase());
        // BookAction.filter({title: searchRef.current!.getInput(),author : author,year : year,tags : tags});
    }
    ...
    return <PageTransition cond={false} className="books-table h-full flex flex-col  overflow-hidden overflowy-scroll gap-4">
        ...
        <div className="flex flex-row align-center justify-between gap-80">
            <SearchBar placeholder="ابحث عن كتاب..." ref={searchRef} onEnter={onSearch} />
        ...
        <div className="flex w-full gap-8 h-0 grow">
            ...
            {/* <Table columns={columns} data={!BooksPageState.filterBooked ? BookState.books : BookState.books.filter(book => !book.available)} /> */}
            {
                (() => {
                    ...
                    return <Table columns={columns} data={books.filter(item => item.title.toLowerCase().includes(filter) || item.author.toLowerCase().includes(filter)|| item.publish_year.toLowerCase().includes(filter))} />;
                })()
            }
        </div>
    </PageTransition>
