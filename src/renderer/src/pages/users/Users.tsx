...
function ReversedBookInfo({ user }: { user: User }) {
    ...
    if (!booked_info || user.reserved_book == null || !BookState.books.length) return <>/</>
    const book_id = booked_info.book_id; //BookingsState.borrowed.find(borrow => borrow.user_id == user.id)?.book_id;
...
function Users() {
    ...
    return <PageTransition cond={false} className="users-table h-full flex flex-col  overflow-hidden overflowy-scroll gap-4">
        ...
        <div className="flex flex-row align-center justify-between gap-80">
            <SearchBar onEnter={onSearch} ref={searchRef} placeholder="ابحث عن عضو..." showFilter={false} />
        ...
        <div className="flex w-full gap-8 h-0 grow">
            {
                (() => {
                    ...
                    return <Table columns={columns} data={users.filter(user => user.first_name.toLowerCase().includes(filter) || user.last_name.toLowerCase().includes(filter))} />
                })()
            }
