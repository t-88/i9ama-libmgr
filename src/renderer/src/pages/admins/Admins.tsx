...
export default function Admins() {
    ...
    return <PageTransition cond={false} className="admins-table h-full flex flex-col  overflow-hidden overflowy-scroll gap-4">
        ...
        <div className="flex flex-row align-center justify-between gap-80">
            <SearchBar onEnter={onSearch} ref={searchRef} placeholder="ابحث عن مسؤول..." showFilter={false} />
        ...
        <div className="flex w-full gap-8 h-0 grow">
            ...
            <Table columns={columns} data={AdminsState.admins.filter(admin => admin.first_name.toLowerCase().includes(filter) || admin.last_name.toLowerCase().includes(filter))} />
        </div>
    </PageTransition>
