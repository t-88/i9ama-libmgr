...
import UsersState, { User, UserAction } from "../../libs/users";
...
export default function BookBookPopup() {
    ...
    function UserItems() {
        function showInfo(e,reserved) {
            ...
            showToast(toast.info,`العضو يملك كتاب ${book.title} لا يمكن حجز اكثر من كتاب للعضو الواحد`);
        }
    ...
    function BookItems() {
        function showInfo(e,reserved) {
            ...
            showToast(toast.info,`الكتاب محجوز من طرف ${user.first_name + ' ' + user.last_name}`);
        }       
        let comps = [];
...
async function onBooking() {
    ...
    showToast(toast.success, "تم حجز الكتاب بنجاح");
